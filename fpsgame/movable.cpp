...
namespace game
{
    ...
    struct movable : dynent
    {
        ...
        void explode(dynent *at)
        {
            state = CS_DEAD;
            exploding = 0;
            game::explode(true, (fpsent *)at, o, this, weapons[WEAP_BARREL].damage, WEAP_BARREL);
        }
        ...
        void damaged(int damage, fpsent *at, int gun = -1)
        {
            ...
            health -= damage;
            if(health>0) return;
            if(gun==WEAP_BARREL) exploding = lastmillis + EXPLODEDELAY;
            else explode(at); 
        }
    ...
    void updatemovables(int curtime)
    {
        ...
        loopv(movables)
        {
            ...
            {
                m->explode(m);
                adddecal(DECAL_SCORCH, m->o, vec(0, 0, 1), weapons[WEAP_BARREL].projradius/2);
            }
