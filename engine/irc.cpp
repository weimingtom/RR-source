...
void ircprintf(ircnet *n, int relay, const char *target, const char *msg, ...)
{
    ...
    if(target && *target && strcasecmp(target, n->nick))
    {
        ...
        if(c)
        {
            ...
            if(n->type == IRCT_RELAY && c->relay >= relay)
                server::sendservmsgf(relay > 1 ? -2 : -3, "\fs\fa[%s]\fS %s", c->friendly, str);
        }
        else
        {
    ...
    conline(0, "%s %s"); // console is not used to relay
}
...
void irccleanup()
{
    ...
    {
        ircnet *n = ircnets[i];
        ircsend(n, "QUIT :%s, %s", "Revelade Revolution", "theintercoolergames.com");
        ircdiscon(n);
    }
}
...
bool ircchangui(g3d_gui *g, ircnet *n, ircchan *c, bool tab, bool front)
{
    ...
    {
        editor *e = useeditor(cwindow, EDITORREADONLY, false);
        if(e) while(c->newlines < c->lines.length()) editorline(e, c->lines[c->newlines++], MAXLINES);
    }
    ...
    {
        irccmd(n, c, v);
        editor *e = useeditor(cinput, EDITORFOREVER, false);
...
bool ircnetgui(g3d_gui *g, ircnet *n, bool tab)
{
    ...
    {
        editor *e = useeditor(window, EDITORREADONLY, false);
        if(e) while(n->newlines < n->lines.length()) editorline(e, n->lines[n->newlines++], MAXLINES);
    }
    ...
    {
        irccmd(n, NULL, w);
        editor *e = useeditor(input, EDITORFOREVER, false);
		useeditor(e->name, e->mode, true);
        e->clear();
    }
