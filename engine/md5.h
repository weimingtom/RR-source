...
struct md5 : skelmodel, skelloader<md5>
{
    ...
    bool loaddefaultparts()
    {
        ...
        fname++;
        defformatstring(meshname)("%s/%s.md5mesh", loadname, fname);
        mdl.meshes = sharemeshes(path(meshname), NULL, 2.0);
        if(!mdl.meshes) return false;
        mdl.initanimparts();
        mdl.initskins();
        defformatstring(animname)("%s/%s.md5anim", loadname, fname);
    ...
    bool load()
    {
        ...
        formatstring(dir)("%s", loadname);
        defformatstring(cfgname)("%s/md5.cfg", loadname);
