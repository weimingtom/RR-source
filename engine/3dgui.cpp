...
struct gui : g3d_gui
{
    ...
    int playerpreview(int model, int team, int weap, float sizescale, bool overlaid)
    {
        ...
        if(visible())
        {
            ...
            if(overlaid)
            {
                ...
                if(!overlaytex) overlaytex = textureload("@{tig/rr-core}/texture/gui/guioverlay.png", 3);
                gle::colorf(1, 1, 1);
    ...
    void line_(int size, float percent = 1.0f)
    {
        if(visible())
        {
            if(!slidertex) slidertex = textureload("@{tig/rr-core}/texture/gui/guislider.png", 3);
        ...
        if(visible())
        {
            ...
            if(icon)
            {
                if(icon[0] != ' ')
                {
                    ...
                    defformatstring(tname)("@{tig/rr-core}/texture/icon/%s%s", icon, ext ? "" : ".png");
    ...
    {
        if(!skintex) skintex = textureload("@{tig/rr-core}/texture/gui/guiskin.png", 3);
